...
def main():
    ...
    def norm(point, H):
        """
        Returns coordinates of point in plane for point in picture
        :param point:
        :return:
        """
        ret = numpy.linalg.inv(H).dot(numpy.append(point, [1]))
        ret /= ret[2]
        return ret[:2]
    ...
    for filename in options.filenames:
        ...
        # l = 1  # numpy.linalg.norm(detection.corners[1] - detection.corners[0])
        # p0 = numpy.array([0., 0])  # detection.corners[0]
        # H = cv2.findHomography(numpy.array([p0, p0 + [0, l], p0 + [l, l], p0 + [l, 0]]),
        #                           detection.corners)[0]
        # H = numpy.array(H)

        # TODO deal with perspective

        H = d0.homography  # TODO this normalizes to square with a = 2
        print abs(d0.homography - d1.homography)

        c0 = [norm(c, H) for c in d0.corners]
        c1 = [norm(c, H) for c in d1.corners]

        w1 = norm(d0.corners[2], H)
        w2 = norm(d1.corners[0], H)

        scale0 = numpy.linalg.norm(c0[1] - c0[0])
        scale1 = numpy.linalg.norm(c1[1] - c1[0])

        print "Scale0: {}".format(scale0)
        print "Scale1: {}".format(scale1)

        print (w2 - w1) / ((scale0 + scale1) / 2) * 45
        ...
        #         corner_norm = norm(corner)
        #         print corner_norm
        #         for other_corner in detection.corners:
        #             print numpy.linalg.norm(norm(other_corner) - corner_norm)
        #             # print numpy.linalg.norm(other_corner - corner)
        #             print
        #
        if len(orig.shape) == 3:
            overlay = orig / 2 + dimg[:, :, None] / 5
        else:
            overlay = gray / 2 + dimg / 5

        if have_cv2:
            cv2.circle(overlay, (int(d0.corners[2][0]), int(d0.corners[2][1])), 3, (0,255,0), -1)
            cv2.circle(overlay, (int(d1.corners[0][0]), int(d1.corners[0][1])), 3, (0,255,0), -1)
            # for corner in d0.corners:
            #     print corner
            #     cv2.circle(overlay, (int(corner[0]), int(corner[1])), 3, (0,255,0), -1)
